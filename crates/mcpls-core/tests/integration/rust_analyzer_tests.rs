...
///
/// Mirrors `ra_e2e.rs`'s `find_line` helper, so an edit to the fixture can't
/// silently desync a hardcoded line/column pair from the symbol it targets.
fn find_line(file: &Path, needle: &str) -> u32 {
    let content = std::fs::read_to_string(file).expect("failed to read file for anchor search");
    content
        .lines()
        .enumerate()
        .find_map(|(i, line)| {
            line.contains(needle)
                .then(|| u32::try_from(i + 1).expect("line number fits u32"))
        })
        .unwrap_or_else(|| panic!("anchor '{needle}' not found in {}", file.display()))
}

/// Find the 1-based line and column of `symbol`'s first occurrence on the line
/// matched by `needle`. The column is a byte offset + 1, which is only correct
/// for ASCII text; fine for this fixture's anchor lines, not general UTF-8 safe.
fn find_anchor(file: &Path, needle: &str, symbol: &str) -> (u32, u32) {
    let line = find_line(file, needle);
    let content = std::fs::read_to_string(file).expect("failed to read file for anchor search");
    let line_text = content
        .lines()
        .nth((line - 1) as usize)
        .unwrap_or_else(|| panic!("line {line} out of range in {}", file.display()));
    let col = line_text
        .find(symbol)
        .unwrap_or_else(|| panic!("symbol '{symbol}' not found on anchor line '{needle}'"));
    (line, u32::try_from(col + 1).expect("column fits u32"))
}

/// Timeout for [`wait_for_std_lib_ready`]'s probe only, Windows-aware because Windows
/// CI runners index noticeably slower, overridable via `MCPLS_RA_INDEX_TIMEOUT_SECS`.
/// Mirrors `ra_e2e.rs`'s `wait_until_ready` timeout policy.
///
/// `wait_for_indexing_ready`'s pre-existing local-crate probe deliberately keeps its
/// original flat 30s timeout instead of this one: it was already reliable at that
/// budget, and 16 other tests call it whose nextest slow-timeout window was never
/// widened for this fix, so raising their timeout without also raising their kill
/// window would risk a new failure mode for tests this issue never touched.
fn std_lib_readiness_timeout() -> Duration {
...
/// Poll `handle_hover` at `position` (1-based `(line, col)`) in `file` until every
/// substring in `required` is present in 3 consecutive responses, guarding against
/// transient results during rust-analyzer's intermediate indexing phases (observed
/// on Windows CI).
///
/// `handle_hover` itself gates on the translator's own indexing-readiness wait
/// (`translator::navigation::INDEXING_READY_TIMEOUT`, up to 30s) before issuing the LSP
/// request, so a single call here can absorb a large chunk of `timeout` -- callers with
/// a short timeout budget should account for that overrun.
async fn wait_for_hover_ready(
    ...
    println!(
        "[rust_analyzer_tests] waiting for {label} readiness (timeout {}s)…",
        timeout.as_secs()
    );
    ...
    loop {
            ...
            Ok(result) => {
                ...
                if required.iter().all(|needle| text.contains(needle)) {
                    ...
                    if consecutive >= required_consecutive {
                        println!("[rust_analyzer_tests] {label} is ready");
                        return;
                    }
                } else {
                    consecutive = 0;
        ...
        if last_print.elapsed() >= Duration::from_secs(10) {
            println!("[rust_analyzer_tests] still waiting on {label}: consecutive={consecutive}");
            last_print = Instant::now();
        }

        if Instant::now() >= deadline {
            tracing::warn!(
                "Timed out waiting for {label} readiness after {}s",
                timeout.as_secs()
            );
            return;
        }
...
///
/// This mirrors the approach in `ra_e2e.rs` and is more reliable than waiting for
/// `publishDiagnostics`, which can arrive before type-checking is complete.
async fn wait_for_indexing_ready(translator: &Arc<Mutex<Translator>>, workspace: &Path) {
    let lib_rs = workspace.join("src/lib.rs");
    let anchor = find_anchor(&lib_rs, "pub fn add(", "add");
    wait_for_hover_ready(
        translator,
        &lib_rs,
        "local-crate (add fn)",
        anchor,
...
/// Poll hover on the `String` field of `User` until rust-analyzer returns
/// consistent std-lib results. `wait_for_indexing_ready` only confirms local-crate
/// symbols are indexed, which does not guarantee sysroot/std-lib indexing has
/// caught up — this closes that gap before a test hovers a std-lib symbol.
///
/// Returns the resolved `(line, col)` anchor so callers can reuse it for their own
/// hover assertion instead of re-reading the fixture file to re-derive it.
async fn wait_for_std_lib_ready(
    translator: &Arc<Mutex<Translator>>,
    workspace: &Path,
) -> (u32, u32) {
    let lib_rs = workspace.join("src/lib.rs");
    let anchor = find_anchor(&lib_rs, "pub name: String,", "String");
    wait_for_hover_ready(
        translator,
        &lib_rs,
        "std-lib (String field)",
        anchor,
        &["String"],
        std_lib_readiness_timeout(),
    )
    .await;
    anchor
}
...
async fn test_hover_on_std_vec() {
    ...
    wait_for_indexing_ready(&translator, &rust_workspace_path()).await;
    // Local-crate readiness doesn't guarantee std-lib/sysroot indexing is done.
    // Reuse the anchor the readiness probe already resolved for the hover assertion below.
    let (string_line, string_col) =
        wait_for_std_lib_ready(&translator, &rust_workspace_path()).await;

    // Hover over "String" in the `User` struct's `name` field.
    let result = timeout(
