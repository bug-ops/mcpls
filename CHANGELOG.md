...
- **`Translator` API** — Breaking change: `lsp_clients`/`lsp_servers` are now keyed by `ServerId` instead of a raw language string; `register_client`/`register_server` take `impl Into<ServerId>`; `set_expected_languages`/`clear_expected_languages` renamed to `set_expected_servers`/`clear_expected_servers` and take `HashSet<ServerId>`; new `with_router`, `rebind_router`, and `router()` methods. (#174)
