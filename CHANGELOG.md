...
- Replace unmaintained `lsp-types` (gluon-lang) with `gen-lsp-types` 0.11.0 (pinned); `lsp_types::` import paths are unchanged, but `Uri` is now an opaque string with no validating parse (construction is infallible, so a malformed MCP-supplied URI in `get_incoming_calls`/`get_outgoing_calls` now surfaces as a file-not-found error instead of an invalid-parameter error), and `LspClient::request_typed` now binds ~15 LSP request call sites' method string and result type to a single `lsp_types::Request` impl. Two user-visible tool-output changes: `search_workspace_symbols` now drops symbols the server reported without a location range instead of fabricating a `(1,1)` placeholder range for them, and `rename_symbol` now silently drops any LSP-3.18 snippet-shaped edit instead of passing its literal placeholder syntax (e.g. `${1:name}`) through as plain replacement text. (#297)
