...
- Project-marker recursive search now correctly respects `.gitignore` again: `standard_filters(false)` was applied after `git_ignore(true)`, silently negating it and letting the walk descend into gitignored directories. (#476)
- Caller-fault errors (`InvalidToolParams`, `PathOutsideWorkspace`, `NotARegularFile`, `InvalidUri`, `DocumentNotFound`, `FileSizeLimitExceeded`, and a `FileIo` caused by a nonexistent path) now map onto JSON-RPC `-32602` (`INVALID_PARAMS`) instead of the generic `-32603` (`INTERNAL_ERROR`); `ServerInitializing` and `WorkspaceServersInitializing` now get their own retryable JSON-RPC code with a structured `data` payload, matching `WorkspaceIndexing`. `read_resource`/`subscribe`'s workspace-roots-empty rejection (`NoWorkspaceRoots`, a server misconfiguration, not caller-fault) now correctly maps to `-32603` instead of the `-32602` those two handlers previously returned for it by mistake; `DocumentLimitExceeded` (aggregate tracker state, not a per-request bad parameter) stays on `-32603` too. (#479)
