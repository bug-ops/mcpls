...
- **`kind` fields on `get_document_symbols`, `workspace_symbol_search`, `get_completions`, and `get_inlay_hints`** — Breaking change: now the raw LSP numeric kind (`u32`) instead of a lossy `Debug`-string rendering or an `Option<u8>` roundtrip that silently dropped custom inlay-hint kinds above 255; `workspace_symbol_search`'s `kind_filter` now also accepts that numeric value (in addition to the existing name form) so a client can filter on the `kind` a result actually carries. `prepare_call_hierarchy`/`get_incoming_calls`/`get_outgoing_calls`'s `kind` field was already numeric but could previously fabricate `0` on a conversion failure that could not actually occur; it no longer can. (#467)
