...
- Navigation, hover, diagnostics, and other position-bearing MCP results now carry a `positions_degraded` flag, set whenever a returned position's column could not be converted for encoding (disk-read budget exhausted, unresolvable server path, oversized file, invalid UTF-8, or a line past EOF), so a caller can tell when returned columns may be unreliable instead of silently receiving a mismatched value. (#497, #502)
- `DocumentTracker` now evicts the least-recently-used, unlocked document (sending `textDocument/didClose` to the LSP server) instead of permanently refusing every new document once `workspace.max_documents` is reached, so a long-running session no longer wedges after opening the configured file limit. (#495, #502)
...
- Fixed a `u32` overflow in `lsp_to_mcp_position` when a routed LSP server reports `line`/`character` at or near `u32::MAX` (a valid LSP 3.17 "end of line" sentinel); it now saturates instead of panicking or silently wrapping to 0. (#426)
...
- `rename_symbol` and `get_code_actions` results now carry a `dropped` field with a per-reason count whenever `convert_workspace_edit` withholds a `WorkspaceEdit` entry (out-of-workspace URI, unsupported file-operation change, or unsupported snippet edit), so an MCP client can tell "nothing to rename" apart from "some of the rename was withheld" instead of relying on log output alone. (#475, #483)
...
- `resources/subscribe`/`unsubscribe` now reject requests served over rmcp's stateless per-request HTTP path with an explicit error instead of silently dropping the subscription. (#482, #490)
...
- Project-marker recursive search now correctly respects `.gitignore` again: `standard_filters(false)` was applied after `git_ignore(true)`, silently negating it and letting the walk descend into gitignored directories. (#476, #485)
- Caller-fault errors (`InvalidToolParams`, `PathOutsideWorkspace`, `NotARegularFile`, `InvalidUri`, `DocumentNotFound`, `FileSizeLimitExceeded`, and a `FileIo` caused by a nonexistent path) now map onto JSON-RPC `-32602` (`INVALID_PARAMS`) instead of the generic `-32603` (`INTERNAL_ERROR`); `ServerInitializing` and `WorkspaceServersInitializing` now get their own retryable JSON-RPC code with a structured `data` payload, matching `WorkspaceIndexing`. `read_resource`/`subscribe`'s workspace-roots-empty rejection (`NoWorkspaceRoots`, a server misconfiguration, not caller-fault) now correctly maps to `-32603` instead of the `-32602` those two handlers previously returned for it by mistake; `DocumentLimitExceeded` (aggregate tracker state, not a per-request bad parameter) stays on `-32603` too. (#479, #485)
- `LspServerError` now maps onto JSON-RPC `-32602` (`INVALID_PARAMS`) when its message is the recognized "position out of range for this document" shape, instead of always `-32603` (`INTERNAL_ERROR`); a resource-subscription limit reached (`SubscriptionError::LimitReached`) now goes through the same caller-fault/server-fault classifier as every other error instead of being hand-mapped to `INVALID_PARAMS`, and stays on `-32603` like `DocumentLimitExceeded`. (#496, #501)
- `unsubscribe` now resolves a stale request URI (e.g. the file was deleted since subscribing) against the alias `subscribe` recorded for it, instead of falling back to the raw URI as a subscription-set key that was never actually recorded under — closing a leaked, un-removable subscription slot. (#499, #501)
