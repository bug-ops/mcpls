...
- **`read_resource` response shape distinguishes untracked from tracked-but-clean diagnostics** — Breaking change: the response, previously `null | {"uri": ..., "version": ..., "diagnostics": [...]}` (a direct serialization of the internal cache entry), is now always an object: `{"tracked": bool, "version": number | null, "diagnostics": [...]}`. `tracked` lets a client tell "this file was never opened by the LSP client" (`tracked: false`) apart from "this file is open but has no diagnostics to report, whether clean or not yet analyzed" (`tracked: true, diagnostics: []`) — previously both serialized as `null` and were indistinguishable. `tracked` is derived from `Translator::is_document_open`, not from diagnostics-cache presence, since a tracked file with an evicted or not-yet-populated cache entry is not observably different from an untracked one by cache lookup alone. `version` (the document version diagnostics were computed against — the client's staleness signal) is carried over unchanged; `uri` is dropped, since the caller already supplied it in the request. (#132)
