...
- `DocumentTracker` now evicts the least-recently-used, unlocked document (sending `textDocument/didClose` to the LSP server) instead of permanently refusing every new document once `workspace.max_documents` is reached, so a long-running session no longer wedges after opening the configured file limit. (#495)
