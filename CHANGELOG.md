...
- In-flight LSP requests now fail immediately with `Error::ServerTerminated` when the message loop exits (transport error, reader task gone, or shutdown), instead of each caller blocking for the full request timeout. (#458)

### Security

- LSP frame header parsing is now bounded on both single-line length and header count per frame, closing an unbounded-memory-growth path from a malicious or malfunctioning spawned LSP server. (#457)
