...
- **`LspTransport::new`** — Breaking change: now returns `(LspTransport, LspTransportReader)` instead of one value; `LspClient`'s inbound read is cancel-safe, fixing LSP stream desync. (#451)
