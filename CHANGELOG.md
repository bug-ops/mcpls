...
- `unsubscribe` now resolves a stale request URI (e.g. the file was deleted since subscribing) against the alias `subscribe` recorded for it, instead of falling back to the raw URI as a subscription-set key that was never actually recorded under — closing a leaked, un-removable subscription slot. (#499)
