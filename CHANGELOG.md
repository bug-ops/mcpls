...
- `rename_symbol` and `get_code_actions` results now carry a `dropped` field with a per-reason count whenever `convert_workspace_edit` withholds a `WorkspaceEdit` entry (out-of-workspace URI, unsupported file-operation change, or unsupported snippet edit), so an MCP client can tell "nothing to rename" apart from "some of the rename was withheld" instead of relying on log output alone. (#475)
