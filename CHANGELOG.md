...
- **`kind` fields on `get_document_symbols`, `workspace_symbol_search`, `get_completions`, and `get_inlay_hints`** — Breaking change: now the raw LSP numeric kind (`u32`) instead of a lossy `Debug`-string rendering or a narrowing `Option<u8>`; `workspace_symbol_search`'s `kind_filter` now also accepts that numeric value. (#467)
